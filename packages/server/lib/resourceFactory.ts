...
import DuplexCollection from "./DuplexCollection";
...
export default class ResourceFactory {
	...
	*/
	 ...
	 */
	createCollectionsForTree(tree: { resources: { pathMappings: any }, dependencies: any[], metadata: { name: string; } }, {
		//@ts-ignore
		getProjectExcludes, getVirtualBasePathPrefix, virtualReaders : any
	} = {}) {
		...
		const dependencyCollection: ReaderCollectionPrioritized[] = [];
		const dependencyPathIndex : any = {};
		const virtualReaderIndex : any = {};
		const sourceResourceLocators = [];

		const that = this;
		function processDependencies(project: { resources: { pathMappings: { [x: string]: any; }; }; path: string; metadata: { name: string | number; }; dependencies: any[]; }) {
			if (project.resources && project.resources.pathMappings) {
				...
				for (const virBasePath in project.resources.pathMappings) {
					if (hasOwnProperty.call(project.resources.pathMappings, virBasePath)) {
						...
						filePaths.push({project: project, virBasePath: virBasePath, getProjectExcludes: getProjectExcludes, getVirtualBasePathPrefix: getVirtualBasePathPrefix})
						// // Create an fs adapter for every path mapping
						// const fsAdapter = that._createFsAdapterForVirtualBasePath({
						// 	project,
						// 	virBasePath,
						// 	getProjectExcludes,
					...
					dependencyCollection.push(...filePaths);
				
			}

			project.dependencies.forEach(function(depProject: any) {
		...
		// 			// Create an fs adapter for every path mapping
		// 			const fsAdapter = this._createFsAdapterForVirtualBasePath({
		// 				project: tree,
		// 				virBasePath,
		// 				getProjectExcludes,
		// 				getVirtualBasePathPrefix
		...
		tree.dependencies.forEach(function(project: any) {
	...
	_createFsAdapterForVirtualBasePath({
		...
		if (getVirtualBasePathPrefix) {
			...
			if (virBasePathPrefix) {
				...
				if (pathExcludes) {
					const normalizedPatterns = pathExcludes.map((pattern: any) => {
						return this._prefixGlobPattern(pattern, virBasePathPrefix);
					});
	 ...
	 * @returns {Promise<string[]>} Promise resolving to list of normalized glob patterns
	 */
	_prefixGlobPattern(virPattern: any, virBaseDir: string) {
	 ...
	 */
	createResource(parameters: { path: any; statInfo: any; buffer: any; string: any; createStream: any; stream: any; project: any; }) {
